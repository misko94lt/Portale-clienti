    ...
    return (React.createElement("div", { style: { minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px", background: `radial-gradient(circle at 50% 0%, ${C.accent}11 0%, ${C.bg} 60%)` } },
        React.createElement("div", { className: "fade-in", style: { maxWidth: 520, width: "100%" } },
            ...
            React.createElement("div", { style: { padding: "14px 16px", background: C.card2, border: `1px solid ${C.border}`, borderRadius: 10, fontSize: 11, color: C.text2, lineHeight: 1.6 } },
                ...
                "I dati restano sul tuo dispositivo. Niente viene inviato a server esterni. Il file lo riceverai direttamente dal tuo tecnico, anche tramite WhatsApp o email."))));
};
/* ═══ STAT CARD ═══════════════════════════════════════════════ */
...
const App = () => {
    const [data, setData] = useState(() => {
        try {
            const saved = sessionStorage.getItem("portale-data");
    ...
    const reset = () => {
        try {
            sessionStorage.removeItem("portale-data");
        }
        catch (e) { }
        setData(null);
    };
    if (!data)
        return React.createElement(Landing, { onLoad: setData });
    return React.createElement(Dashboard, { data: data, onReset: reset });
};
